...
def creat_graph(args):
    if args.input[-4:] == ".mat":
        ...
        # load edgelist and labels
        graph_dict = struct2vec.load_edgelist(args.input, undirected = True)
        # create networkx graph objects
        A=nx.Graph()
        for key in graph_dict.keys():
            A.add_node(key)
        for key in graph_dict.keys():
            for element in graph_dict[key]:
                A.add_edge(key,element)
        ...
        colors = ((np.arange(4) == labels[...,None]).astype(int))
        # pdb.set_trace()
    return A, colors
